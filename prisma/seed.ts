  ...
  if (!existingProfile) {
    await prisma.businessProfile.create({
      data: {
        ...
        customNotes:
          'Fleet includes Executive Sedans (Mercedes E-Class, BMW 7-Series, Audi A8), Luxury SUVs (Audi Q7, Lexus RX), Mercedes V-Class People Movers, and Stretch Limousines.',
      },
    });
...
Email: bookings@opalchauffeurs.com.au | Direct: +61 400 000 000`,
  ...
  // Seeded in a genuine "never run yet" state — no fabricated summaries or
  // item counts. On re-seed, only structural fields are touched so real
  // accumulated run history is never overwritten back to these placeholders.
  for (const b of backgroundJobsData) {
    await prisma.backgroundJob.upsert({
      where: { jobType: b.jobType },
      update: { title: b.title, frequency: b.frequency },
      create: b,
    });
  }
